...
/// FactSet represents a collection of [Fact] objects and provides methods for
...
public interface FactSet extends Iterable<Fact> {
    ...
    /// Add new function in addition to the existing [#factOperation()], that takes the original fact and changes it.
    ...
    /// Retrieve [Fact]s from this [FactSet] that are transformed first by applying [#factOperation()].
    ///
    ...
    /// Retrieves a specific part of the [FactSet] by name.
    ///
    ...
    /// Provides part names contained within the [FactSet].
    ///
    /// @return an Iterable of part names.
    Set<String> parts();

    /// Overwrite the part names contained within the [FactSet].
    /// @param newName the new name for the part.
    /// @return the factset (shallow) copy with a new name
    FactSet setPart(String newName);

    /// Get plain a [Map] where the key is the part name and the values are a list of facts.
    ///
    /// @return a new map.
    ...
    /// Filters the [FactSet] using the provided predicate.
    ///
    ...
    /// Groups the [FactSet] by the specified function.
    ///
    ...
    /// Joins the current [FactSet] with another [FactSet] based on the specified key functions.
    ///
    ...
    /// Returns a new [FactSet] containing only unique elements from the original set.
    ///
    /// @return a distinct FactSet.
    FactSet distinct();

    /// Combines the current [FactSet] with another [FactSet].
    ///
    ...
    /// Returns a new [FactSet] containing only the elements that are present in both the current
    ...
    /// Returns a sequential [Stream] with the collection of [Fact] elements as its source.
    ///
    /// @return a Stream of Fact elements.
    Stream<Fact> stream();

    /// Returns the number of elements in the [FactSet].
    ///
    /// @return the size of the FactSet.
    long size();

    /// Checks if the [FactSet] is empty.
    ///
    /// @return `true` if the FactSet is empty; `false` otherwise.
    boolean isEmpty();

    /// Checks if any facts in the [FactSet] match the given predicate.
    ///
    /// @param predicate the predicate to apply.
    /// @return `true` if any facts match the predicate; `false` otherwise.
    boolean any(Predicate<Fact, FactSet> predicate);

    /// Checks if all facts in the [FactSet] match the given predicate.
    ///
    /// @param predicate the predicate to apply.
    /// @return `true` if all facts match the predicate; `false` otherwise.
